import heroImage from "../../imports/woman-playing-with-dog-on-sandy-beach-2026-01-08-06-39-41-utc.JPG";
import { Link } from "react-router";
import { Star, ArrowRight, MessageCircle } from "lucide-react";
import BookingCTA from "../components/BookingCTA";
import SEO from "../components/SEO";
import emilyImage from "../../imports/Gemini_Generated_Image_lfgepqlfgepqlfge.png";
import portfolioFamilies from "../../imports/outdoor-shot-of-pleased-man-and-woman-stand-closel-2026-05-28-23-39-35-utc.JPG";
import portfolioPets from "../../imports/dog-in-autumn-foliage-wearing-a-red-collar-2026-03-25-00-44-16-utc.jpg";
import portfolioBranding from "../../imports/japanese-woman-in-office-portrait-2026-03-09-05-22-48-utc.jpg";
import portfolioEvents from "../../imports/celebrating-together-at-an-office-new-year-s-party-2026-01-09-09-10-33-utc.jpg";

const homeSchema = {
  "@context": "https://schema.org",
  "@type": "LocalBusiness",
  "@id": "https://www.ehrayphotography.com",
  "name": "EHRoy Photography",
...
const testimonials = [
  { name: "Sarah Al-Mahmoud", role: "Mother of three, Dubai", text: "Emily sees the moments you miss when you're living them. Our photographs don't look like a session - they look like our life.", rating: 5 },
  { name: "James & Priya Nair", role: "Family session, Abu Dhabi", text: "We were nervous in front of the camera. Within ten minutes we forgot it was there. The results were beyond anything we expected.", rating: 5 },
  { name: "Layla Osman", role: "Personal branding, Dubai", text: "My brand presence changed overnight. Clients now comment on my photos before anything else. Emily understood my brief immediately.", rating: 5 },
];
...
export default function Home() {
  return (
    <>
      <SEO
        title="EHRoy Photography Dubai | Natural Light Lifestyle Photography UAE"
      ...
      <div className="relative">
        <div className="sticky top-0 h-screen overflow-hidden bg-muted" style={{ marginBottom: "-100vh", zIndex: 0 }}>
          <img src={heroImage} alt="A woman and her dog on a sandy beach - EHRoy Photography, UAE" className="w-full h-full object-cover object-center" />
        ...
        <section className="relative h-screen min-h-[640px] flex items-end pb-20" style={{ zIndex: 1 }}>
          <div className="max-w-7xl mx-auto px-6 w-full">
            <p className="text-white/60 text-[10px] tracking-[0.35em] uppercase mb-5 font-medium">EHRoy Photography &nbsp;·&nbsp; United Arab Emirates</p>
        ...
        <section className="relative py-28" style={{ zIndex: 1, background: "rgba(0,0,0,0.82)" }}>
          <div className="max-w-7xl mx-auto px-6 grid md:grid-cols-2 gap-16 lg:gap-24 items-center">
            <div>
              ...
              <p className="text-white/65 leading-relaxed mb-5 text-[15px]">Most photographers compete on editing. EHRoy Photography competes on something harder to fake - authenticity. The most powerful images aren't constructed. They're caught.</p>
            ...
            <div className="pl-0 md:pl-6">
              <div className="border-l-[3px] border-white/30 pl-8 py-2">
                ...
                <p className="mt-6 text-white/40 text-xs tracking-wider uppercase">- Emily Ray, Founder of EHRoy Photography</p>
              </div>
            </div>
          </div>
        </section>
        ...
        <section className="relative py-14" style={{ zIndex: 1, background: "rgba(0,0,0,0.68)", borderTop: "1px solid rgba(255,255,255,0.08)" }}>
          <div className="max-w-7xl mx-auto px-6 grid grid-cols-2 md:grid-cols-4 gap-8 md:gap-4">
            {[{ value: "8+", label: "Years of Experience" }, { value: "1,400+", label: "Sessions Completed" }, { value: "4.9 ★", label: "Google Rating" }, { value: "UAE", label: "Based - Global Availability" }].map(stat => (
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            {portfolio.map(item => (
              <Link key={item.label} to={item.path} className="group relative overflow-hidden bg-muted rounded-3xl block" style={{ aspectRatio: "3/4" }}>
                <img src={item.image} alt={`${item.label} photography by EHRoy - UAE`} loading="lazy" className="w-full h-full object-cover transition-transform duration-1000 ease-out group-hover:scale-[1.06]" />
      ...
      <section className="py-28 bg-secondary">
        <div className="max-w-7xl mx-auto px-6">
          <div className="mb-14">
            <p className="text-muted-foreground text-[10px] tracking-[0.35em] uppercase mb-3 font-medium">Client Experiences</p>
            <h2 className="text-3xl sm:text-4xl font-medium text-foreground max-w-xl leading-tight" style={{ fontFamily: "'Lora', Georgia, serif" }}>What clients say</h2>
          </div>
        ...
        <div className="max-w-7xl mx-auto px-6 grid md:grid-cols-2 gap-14 lg:gap-24 items-center">
          <div className="relative order-2 md:order-1">
            <div className="aspect-[3/4] bg-muted overflow-hidden rounded-3xl max-w-md">
              <img src={emilyImage} alt="Emily Ray, founder of EHRoy Photography, with cameras in a mountain landscape at golden hour" className="w-full h-full object-cover object-[center_20%]" />
            </div>
          </div>
